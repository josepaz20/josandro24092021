...
function verActivar(idUsuario) {
    $.get('activar', {idUsuario: idUsuario}, setFormulario);
    bloqueoAjax();
}
function verActualizar(idUsuario) {
    $.get('actualizar', {idUsuario: idUsuario}, setFormulario);
    bloqueoAjax();
}

function verEliminar(idUsuario) {
    $.get('eliminar', {idUsuario: idUsuario}, setFormulario);
    bloqueoAjax();
}
function vercambiarContrasena(idUsuario) {
    $.get('cambiarcontrasena', {idUsuarios: idUsuario}, setFormulario);
    bloqueoAjax();
}
...
function validarCambiarContrasena() {
    if (confirm(" DESEA CAMBIAR LA CONTRASEÑA DEL USUARIO?")) {
        bloqueoAjax();
        return true;
    } else {
        return false;
